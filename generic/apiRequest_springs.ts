...
import { Authorization, SPRINGS, currentUser, stage } from '../meta';
...
export const apiRequestCollectionLogDetails = (async (flatID : string) => {
    ...
    const url = `https://${stage}.crmadmin.inncircles.com/api/collection-log/${flatID}`;  
    const response = await axios.get(url ,{ headers : headers});
    return ( response.data.data.flat);
}); 

export const apiRequestSaleParticulars = (async () => {
    const url = `https://${stage}.crmadmin.inncircles.com/api/collection-log?sortBy=%7B%22flatId%22:1%7D&filter=%7B%7D`;  
    const response = await axios.get(url ,{ headers : headers});
...
export const apiRequestmcd = (async () => {
    const url = `https://${stage}.crmadmin.inncircles.com/api/mcd-screen?sortBy=%7B%22_id%22:1%7D&filter=%7B%7D`;  
    const response = await axios.get(url ,{ headers : headers});
