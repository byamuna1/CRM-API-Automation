...
import { apiRequestSaleParticulars } from '../generic/apiRequest_spire';
import { EXCELS, RESPONSE, SHEETS, EXCELJS, HEADERS, PATH } from '../constants';
import { SPIRE } from '../meta';
import { createFolder } from '../generic/apiRequest_spire';
const fs = require('fs')
let flatDetails : any = {};
...
test ("spire Sale Particulars" , async () => {
    ...
    const saleparticulars = workbook1.addWorksheet(EXCELS.SPIRE);
    const flat_mis = workbook2.addWorksheet(EXCELS.SPIRE);
    ...
    for(let i=6;i<rowcount;i++)
    {
        ...
        grossAmountDetails = {
                ...
                totalAmount : row.getCell(46).value?.result??0,
        }
