...
import { EXCELS, RESPONSE, SHEETS, EXCELJS, HEADERS, PATH, BANK, AXIS, AXIS_PRASANNA, AXIS_RAJESH, BHFL, HDFC, HDFC_LTD } from '../constants';
...
test ("spire costsheet Data" , async () => {
    ...
    const bank_details = workbook2.addWorksheet(EXCELS.SPIRE)
    ...
    for(let i=6; i<rowcount; i++)
    {
        ...
        costSheetDetailsForomScr = {
                ...
                bankName : row.getCell(79).value,
                bankPOCName : row.getCell(80).value,
    ...
    for(let i=4; i< rowCount1; i++)
    {
        ...
        {
            //console.log(bankDetailsFromScr)
    ...
    for(let index=0; index<res.data.length ; index++)
    {
        ...
        if(scrCostSheet.has(String(result.data[RESPONSE.FLATNUMBER])))
        {
            ...
            {
                let bank_sheet = scrCostSheet.get(String(result.data[RESPONSE.FLATNUMBER]))
                    ...
                    else
                    {
                        ...
                        else if(bankFromScr.includes(BANK.HDFC))
                        {
                            ...
                            if(result.data.loanDetails['poc'])
                            {
                                ...
                                else if(bankFromScr == 'hdfc ltd' || bankFromScr == 'hdfc-others' )
                                {
