...
import { apiRequestReceiptLogs, apiRequestmcd, createFolder } from '../generic/apiRequest_spire';
...
test ("spire ReceiptLogs Details" , async () => {
    ...
    {
        mcd_system.set(String(parseInt(response[i].flatNo)),{'sNo':count_system++ ,'flatNumber' : response[i].flatNo , 'corpus_amount' : response[i].corpus.gross , 'maintaince_amount': response[i].maintenance.gross, 'movein_amount' :  response[i].moveIn.gross , 'corpus_collected' :  response[i].corpus.collected , 'maintaince_collected' :  response[i].maintenance.collected, 'movein_collected' :  response[i].moveIn.collected})
    }
