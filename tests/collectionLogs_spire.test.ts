...
test ("Spectra ReceiptLogs Details" , async () => {
    ...
    for(let i = 6;i<5000;i++)
    {
        ...
        if(receiptDetails.milestone != 'Lumpsum' && receiptDetails.receiptNo != null && receiptDetails.amount != null && receiptDetails.milestone != 'Cancel')
        {
    ...
    const filePath1 = path.join(__dirname, EXCELS.SPIRE_MISSING_RECEIPTS);
